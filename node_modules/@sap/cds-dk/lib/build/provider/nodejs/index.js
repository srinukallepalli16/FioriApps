...
class NodejsBuildPlugin extends EdmxBuildPlugin {
    ...
    async #cleanupPackageJson() {
        ...
        try {
            const packageJson = await cds.utils.read(packageJsonPath)
            ...
            if (modified) {
                await cds.utils.write(packageJsonPath, JSON.stringify(packageJson, null, 2), 'utf-8')
                // update the package-lock.json if it exists
                if (cds.utils.exists(path.join(this.task.dest, 'package-lock.json'))) {
